...
void GS_ParticleRender(point VS_OUT _in[1], inout TriangleStream<GS_OUT> _OutStream)
{
    ...
    float rotc = ParticleBuffer[_in[0].iInstance].vDir.x;
    float rots = ParticleBuffer[_in[0].iInstance].vDir.y;
    //float3 NewPos[4] =
    //{
    ...
    float3 NewPos[4] =
    {
        vViewPos + float3(-0.5f * rotc * vScale.x + 0.5f * -rots * vScale.y, -0.5f * rots * vScale.x + 0.5f * rotc * vScale.y, 0.f),
        vViewPos + float3(0.5f * rotc * vScale.x + 0.5f * -rots * vScale.y, 0.5f * rots * vScale.x + 0.5f * rotc * vScale.y, 0.f),
        vViewPos + float3(0.5f * rotc * vScale.x + -0.5f * -rots * vScale.y, 0.5f * rots * vScale.x + -0.5f * rotc * vScale.y, 0.f),
        vViewPos + float3(-0.5f * rotc * vScale.x + -0.5f * -rots * vScale.y, -0.5f * rots * vScale.x + -0.5f * rotc * vScale.y, 0.f)
    };
