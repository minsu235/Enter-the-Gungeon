...
void CRenderMgr::UpdateLight2D()
{
	...
	{
		UINT size = m_pLight2DBuffer->GetElementSize();
		delete m_pLight2DBuffer;
		m_pLight2DBuffer = new CStructuredBuffer();
		m_pLight2DBuffer->Create(size, (UINT)m_vecLight2D.size(), SB_TYPE::SRV_ONLY, nullptr, true);
	}
...
CCamera* CRenderMgr::GetMainCam()
{
	...
	{
		if (m_mainCam != nullptr && CLevelMgr::GetInst()->GetCurLevel()->GetName() == L"Stage")
		{
			return m_mainCam;
		}
		if (m_vecCam.empty())
